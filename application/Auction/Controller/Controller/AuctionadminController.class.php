...
class AuctionadminController extends AdminbaseController
{
    ...
    public function auctionindex()
    {
        ...
        $list = $this->pmzt_model
            ...
            ->field('id,adduser,tname,username,addtime')
            ->select();
...
//            $list[$k]['num'] = $this->pmgezi_model->where(array('cid' => $val['id']))->count();
    ...
    public function auctionadd()
    {
        if (IS_POST) {
			...
			//自动生成缩略图
			$url = './data/upload/';
			$imgArr = explode('/',$_POST['post']['pics']);
			$imgCount = count($imgArr)-1;
			$newImgUrl = '';
			foreach($imgArr as $k=>$val){
				if($k == $imgCount){
					$newImgUrl .= '/thumb_'.$val;
				}elseif($k == 0){
					$newImgUrl .= $val;
				}else{
					$newImgUrl .= '/'.$val;
			...
			thumb_scale($url.$_POST['post']['pics'],$url.$newImgUrl,323,291);
			$_POST['post']['small_pics'] = $newImgUrl;
            $_POST['post']['adduser'] = get_current_admin_id();
			$_POST['post']['username'] = session('name');
			$_POST['post']['is_del'] = 1;
			$_POST['post']['addtime'] = time();
            $article = I("post.post");
    ...
    public function auctiondelete()
    {
        if (isset($_GET['id'])) {
            ...
            if ($this->pmzt_model->where(array('id' => $id))->save(array('is_del'=>0))) {
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
            }
        ...
        if (isset($_POST['ids'])) {
            $ids = I('post.ids/a');
            if ($this->pmzt_model->where(array('id' => array('in', $ids)))->save(array('is_del'=>0))) {
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
            }
