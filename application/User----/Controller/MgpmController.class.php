...
class MgpmController extends MemberbaseController {
	...
	*/
	public function offer() {
		...
		$time = time();
		$newPrice = $margin_price+$price;//新价格
	...
	*/
	public function agent(){
		...
		$time = time();
		//获取代理出价
		$znewPrice = $margin_price+$newPrice;//最新价格
		$pmgeziOfferAgentModWhere = array();
		$pmgeziOfferAgentModWhere['pid'] = $cid;
		...
		if($time-$end_time > 0){
			...
			if($end_time-$offerTime['addtime'] < 481){
				...
				$resources = $pmgeziOfferMod->add($data);
				if(empty($resources)){
					$this->error('代理出价出价失败，请重新代理出价');exit;
				}
				$offerData = array();
				$offerData['pid'] = $cid;
